        ...
        connection.query('select * from friendShip where friendFrom = "' + userID + '" or friendTo = "' + userID + '"', function (err, result) {
            if (err) {
                ...
                p.then(function (friendArr) {
                    ...
                    connection.query('select u.avatar,u.userName,c.circleID,c.imgURL,c.circleTime,c.circleContent  from circle c,userInfo u where c.userID in (' + friendArr + ') and c.userID = u.userID order by c.circleTime desc', function (err, result) {
                        if (err) {
                            throw err;
                        } else {
                            res.send(result)
